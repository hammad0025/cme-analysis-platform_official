...
| Variable | Purpose |
| --- | --- |
| `REACT_APP_API_URL` | Live backend base URL. Leave blank to stay in mock. |
| `REACT_APP_DEV_MODE` | `true` skips Cognito and signs in a local demo user. |
| `REACT_APP_USE_MOCK_API` | `true` simulates uploads and processing locally. |
| `REACT_APP_USER_POOL_ID` / `REACT_APP_USER_POOL_WEB_CLIENT_ID` | Only used when dev mode is `false`. |

When `REACT_APP_DEV_MODE=true` and `REACT_APP_API_URL` is empty, the mock
layer activates automatically. Uploads are simulated (with realistic
per-file progress) and case state persists in `localStorage`. Clearing site
data resets the demo cases.
